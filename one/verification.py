...
def _has_specific_citation(text: str) -> bool:
    """Check if text contains specific citations, not just vague references.

    "Research shows" is NOT a citation. "Smith et al. (2023)" or "doi:10.1234" IS.
    """
    import re as _re
    specific_patterns = [
        r'\bdoi:\s*10\.\d+',
        r'\bPMID:\s*\d+',
        r'\bet al\.\s*[\(,]?\s*\d{4}',
        r'\(\d{4}\)\s',           # Year citations like (2023)
        r'\b[A-Z][a-z]+\s+\(\d{4}\)',  # Author (Year)
        r'\bpublished in\s+\w+',
        r'\bjournal of\b',
        r'\bproceedings of\b',
    ]
    return any(_re.search(p, text, _re.I) for p in specific_patterns)
...
class VerificationEngine:
    ...
    def verify_finding(self, finding_text: str, current_confidence: float = 0.5) -> dict:
        ...
        EPISTEMIC SAFETY: This is LLM self-verification — an LLM checking
        another LLM's work. The confidence ceiling is enforced to prevent
        self-reinforcing confidence inflation. True verification requires
        independent empirical evidence, not LLM agreement.
        """
...
IMPORTANT: You are an LLM verifying text that may have been generated by an LLM.
Your agreement with the finding is NOT independent verification. Only cite SPECIFIC
external evidence (studies, datasets, empirical results). If you cannot cite specific
evidence, mark as UNVERIFIABLE — do not mark as VERIFIED just because it sounds right.

FINDING: {finding_text}

Respond with:
STATUS: [VERIFIED|CORROBORATED|CHALLENGED|UNVERIFIABLE]
NEW_EVIDENCE: [specific external evidence — study names, dates, DOIs, not just "research shows"]
...
REASONING: [brief explanation]"""
        ...
        # EPISTEMIC SAFETY: Apply confidence ceiling for LLM self-verification.
        # An LLM agreeing with itself is not independent verification.
        from .epistemic_safety import (
            apply_confidence_ceiling, classify_provenance, LLM_SELF_VERIFY_CEILING,
        )
        provenance = classify_provenance(finding_text, "research")
        new_confidence = apply_confidence_ceiling(new_confidence, provenance)

        # If the LLM "verified" without citing specific evidence, downgrade
        if status == "verified" and not _has_specific_citation(evidence):
            status = "unverifiable"
            new_confidence = min(new_confidence, current_confidence)
            reasoning += " [SAFETY: LLM self-verification without specific citations downgraded to unverifiable]"
